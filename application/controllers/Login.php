...
class Login extends CI_Controller {
	...
	public function verifylogin($p1='',$p2=''){
		...
		if($status['status']=='1'){
			...
			$gids=$user['gid'];
			$uid=$user['uid'];
			 $sl="select * from quizbox_group where gid in ($gids) ";
			$rq=$this->db->query($sl);
			...
			foreach($gr as $pk => $pv){
				$gid=$pv['gid'];
				$sl2="select * from quizbox_payment where uid='$uid' and gid='$gid' and payment_status='Paid' ";
		...
		public function insert_user()
	{
		...
		$this->form_validation->set_rules('email', 'Email', 'required|is_unique[quizbox_users.email]');
	...
	function authentication ($user, $pass){
                  ...
                  if(empty($user) || empty($pass)){
                    ...
                    // if username already exist in quizbox_users
                    ...
                    $this->db->insert('quizbox_users',$userdata);
