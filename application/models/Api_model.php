...
Class Api_model extends CI_Model
{
	...
	function no_attempted($user){
		$uid=$user['uid'];
 		$query=$this->db->query("select * from quizbox_result where uid='$uid' group by quid");
		return $query->num_rows();						
	}

	function no_pass($user){
		$uid=$user['uid'];
 		$query=$this->db->query("select * from quizbox_result where uid='$uid' and result_status='Pass' group by quid");
		return $query->num_rows();						
	}

	function no_fail($user){
		$uid=$user['uid'];
 		$query=$this->db->query("select * from quizbox_result where uid='$uid' and result_status='Fail' group by quid");
		return $query->num_rows();						
	}
	...
	function get_notification($user,$limit){
		...
		$this->db->select('title,message,notification_date');
 		$this->db->or_where('quizbox_notification.uid',$uid);
		$this->db->or_where('quizbox_notification.uid','0');
	 	//$this->db->limit($this->config->item('number_of_rows'),$limit);
		$this->db->order_by('nid','desc');
		$query=$this->db->get('quizbox_notification');
