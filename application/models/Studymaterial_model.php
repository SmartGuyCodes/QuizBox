...
Class Studymaterial_model extends CI_Model
{
	function studymaterial_list($limit){
		...
		$this->db->join('quizbox_category', 'quizbox_category.cid = study_material.cid', 'left'); 
		$query=$this->db->get('study_material');
		
		return $query->result_array();
	}
 
    function getLandingMaterial(){
        // $query = $this->db->get('study_material');
        // return $query->result_array();
        try{
            $this->db->select('stid, title, attachment, gids, cid, created_date');
            // $this->db->select('*');
	...
	function getstudymaterial_view($stid){
		$this->db->where('study_material.stid',$stid);
	 	$this->db->join('quizbox_category', 'quizbox_category.cid = study_material.cid'); 
		$query=$this->db->get('study_material');
