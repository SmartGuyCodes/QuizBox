...
Class Qbank_model extends CI_Model
{
  ...
  function question_list($limit,$cid='0',$lid='0'){
	$logged_in=$this->session->userdata('logged_in');
	 if($this->input->post('search')){
		 ...
		 $this->db->or_where('quizbox_qbank.qid',$search);
		 $this->db->or_like('quizbox_qbank.question',$search);
		 $this->db->or_like('quizbox_qbank.description',$search);
	 ...
	 if($logged_in['uid'] != '1'){
	 $uid=$logged_in['uid'];
	 $this->db->where('quizbox_qbank.inserted_by',$uid);
	 }
 ...
 function get_option($qid){
	 $this->db->where('qid',$qid);
	 $query=$this->db->get('quizbox_options');
	 return $query->result_array();
 ...
 function remove_question($qid){
	 ...
	 $this->db->where('qid',$qid);
	 if($this->db->delete('quizbox_qbank')){
		  $this->db->where('qid',$qid);
			$this->db->delete('quizbox_options');
			
						
	$qr=$this->db->query("select * from quizbox_quiz where FIND_IN_SET($qid, qids) ");
			...
			$noq=count($nqids);
			$nqids=implode(',',$nqids);
			$this->db->query(" update quizbox_quiz set qids='$nqids', noq='$noq' where quid='$quid' ");
			}
 ...
 function insert_question_1(){
	 ...
	 $this->db->insert('quizbox_qbank',$userdata);
	...
	$userdata=array(
	 ...
	 $this->db->insert('quizbox_options',$userdata);	 
 ...
 function insert_question_5(){
  ...
  function update_question_1($qid){
	 ...
	 $this->db->where('qid',$qid);
	 $this->db->update('quizbox_qbank',$userdata);
	 $this->db->where('qid',$qid);
	$this->db->delete('quizbox_options');
	 foreach($this->input->post('option') as $key => $val){
 ...
 function update_category($cid){
		...
		if($this->db->update('quizbox_category',$userdata)){
 ...
 function remove_category($cid){
	 ...
	 $this->db->where('cid',$cid);
	 if($this->db->delete('quizbox_category')){
		 return true;
	 }else{
 ...
 function insert_category(){
		...
		if($this->db->insert('quizbox_category',$userdata)){
...
// level function start
 function level_list(){
	  $query=$this->db->get('quizbox_level');
	 return $query->result_array();
 ...
 function update_level($lid){
		...
		if($this->db->update('quizbox_level',$userdata)){
 ...
 function remove_level($lid){
	 ...
	 $this->db->where('lid',$lid);
	 if($this->db->delete('quizbox_level')){
		 return true;
	 }else{
 ...
 function insert_level(){
		...
		if($this->db->insert('quizbox_level',$userdata)){
...
$question_type=$this->lang->line('long_answer');	
	...
	if($this->db->insert('quizbox_qbank',$insert_data)){
	...
	$prevtype=$ques_type;
		...
		for($i=1;$i<=10;$i++){
			if($singlequestion[$optionkeycounter] != ""){
				...
				$this->db->insert("quizbox_options",$insert_options);
 ...
 //match Answer
	...
	$this->db->where('qid',$prevqid);
	 $this->db->update('quizbox_qbank',$update_data);
		...
		for($i=1;$i<=10;$i++){
			if($singlequestion[$optionkeycounter] != ""){
				...
				$this->db->update("quizbox_options",$insert_options);
				$optionkeycounter++;
				}
