...
Class Quiz_model extends CI_Model
{
  ...
  function quiz_list($limit,$stat=''){
	  ...
	  $logged_in=$this->session->userdata('logged_in');
			...
			$gid=$logged_in['gid'];
			$uid=$logged_in['uid'];
			$query=$this->db->query("select * from quizbox_users where uid='$uid' ");
 ...
 function insert_quiz(){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->insert('quizbox_quiz',$userdata);
 ...
 function update_quiz($quid){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->update('quizbox_quiz',$userdata);
	  
	  $this->db->where('quid',$quid);
	  $query=$this->db->get('quizbox_quiz',$userdata);
	 $quiz=$query->row_array();
	 if($quiz['question_selection']=='1'){
		 
	  $this->db->where('quid',$quid);
	  $this->db->delete('quizbox_qcl');
                $correct_i=array();
        	 $incorrect_i=array();	 
	 foreach($_POST['cid'] as $ck => $val){
		 ...
		 $this->db->insert('quizbox_qcl',$userdata);
...
/*
	 if($cid!='0'){
		 $this->db->where('quizbox_qbank.cid',$cid);
	 }
	 if($lid!='0'){
		 $this->db->where('quizbox_qbank.lid',$lid);
	 }
*/
	  
	 $query=$this->db->query("select * from quizbox_qbank join quizbox_category on quizbox_category.cid=quizbox_qbank.cid join quizbox_level on quizbox_level.lid=quizbox_qbank.lid 
	 where quizbox_qbank.qid in ($qids) order by FIELD(quizbox_qbank.qid,$qids) 
	 ");
	 return $query->result_array();
 ...
 function get_options($qids){
	 ...
	 $query=$this->db->query("select * from quizbox_options where qid in ($qids) order by FIELD(quizbox_options.qid,$qids)");
	 return $query->result_array();
...
function get_qcl($quid){
 ...
 function remove_quiz($quid){
	 ...
	 $this->db->where('quid',$quid);
	 if($this->db->delete('quizbox_quiz')){
...
$noq=$quiz['noq'];	
	...
	$wqids=implode(',',$qids);
	$noq=array();
	$query=$this->db->query("select * from quizbox_qbank join quizbox_category on quizbox_category.cid=quizbox_qbank.cid where qid in ($wqids) ORDER BY FIELD(qid,$wqids)  ");	
	...
	foreach($qcl as $k => $val){
		...
		$i=0;
	$query=$this->db->query("select * from quizbox_qbank join quizbox_category on quizbox_category.cid=quizbox_qbank.cid where quizbox_qbank.cid='$cid' and lid='$lid' ORDER BY RAND() limit $noq ");	
	$questions=$query->result_array();
	foreach($questions as $qk => $question){
		$qids[]=$question['qid'];
		if(!in_array($question['category_name'],$categories)){
	...
	$zeros=array();
	 ...
	 $this->db->insert('quizbox_result',$userdata);
 ...
 function open_result($quid,$uid){
	 $result_open=$this->lang->line('open');
		$query=$this->db->query("select * from quizbox_result  where quizbox_result.result_status='$result_open'  and quizbox_result.uid='$uid'  "); 
	if($query->num_rows() >= '1'){
		$result=$query->row_array();
return $result['rid'];		
	}else{
 ...
 function quiz_result($rid){
	...
	$query=$this->db->query("select * from quizbox_result join quizbox_quiz on quizbox_result.quid=quizbox_quiz.quid where quizbox_result.rid='$rid' "); 
	return $query->row_array(); 
...
function saved_answers($rid){
	...
	$query=$this->db->query("select * from quizbox_answers  where quizbox_answers.rid='$rid' "); 
	return $query->result_array(); 
 ...
 function assign_score($rid,$qno,$score){
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 $this->db->update('quizbox_result',$userdata);
	 ...
	 $qp=$r_qids[$qno];
		...
		if($$qp_score=='1'){
		  ...
		  $query_qp="update quizbox_qbank set  $crin  where qid='$qp'  ";
 ...
 function submit_result(){
	...
	$query=$this->db->query("select * from quizbox_result 
	join quizbox_quiz on quizbox_result.quid=quizbox_quiz.quid 
	join quizbox_users on quizbox_users.uid=quizbox_result.uid 
	where quizbox_result.rid='$rid' "); 
	$quiz=$query->row_array(); 
	$score_ind=explode(',',$quiz['score_individual']);
	$r_qids=explode(',',$quiz['r_qids']);
	$qids_perf=array();
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 foreach($qids_perf as $qp => $qpval){
		 ...
		 if($qpval=='0'){
		  ...
		  $query_qp="update quizbox_qbank set no_time_served=(no_time_served +1)  $crin  where qid='$qp'  ";
	 $this->db->query($query_qp);
...
if($this->config->item('allow_result_email')){
	...
	$query = $this -> db -> query("select quizbox_result.*,quizbox_users.*,quizbox_quiz.* from quizbox_result, quizbox_users, quizbox_quiz where quizbox_users.uid=quizbox_result.uid and quizbox_quiz.quid=quizbox_result.quid and quizbox_result.rid='$rid'");
	$qrr=$query->row_array();
	...
	$this->db->delete('quizbox_answers');
	 ...
	 foreach($_POST['answer'] as $ak => $answer){
		 ...
		 if($_POST['question_type'][$ak] == '1' || $_POST['question_type'][$ak] == '2'){
			 ...
			 $qid=$qids[$ak];
			 $query=$this->db->query(" select * from quizbox_options where qid='$qid' ");
			 ...
			 $marks=0;
				foreach($answer as $sk => $ansval){
					...
					$this->db->insert('quizbox_answers',$userdata);
				$attempted=1;	
				}
				if($attempted==1){
					if($marks >= '0.99' ){
...
$dir_paths=getcwd();
 $curl = curl_init();  curl_setopt_array($curl, array( CURLOPT_RETURNTRANSFER => 1, CURLOPT_URL => 'https://quizboxquiz.com/license/verify_key.php',  CURLOPT_USERAGENT => 'Codular Sample cURL Request',   CURLOPT_POST => 1,
